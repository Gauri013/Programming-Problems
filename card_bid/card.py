...
def calculate_score(value_one , value_two , diamond_val):
    ...
    if(score_one > score_two):
        ...
        score_player2 += (value.get(diamond_val))/2
    else:
        score_player2 += value.get(diamond_val)
    print("Player one score = ",score_player1)
    print("computer = ",score_player2)

displayed_diamond = random.choice(diamond)
diamond.remove(displayed_diamond)
print("diamond",displayed_diamond)
print ("enter your card of spade")
while (len(diamond) != 0):
    player_one = input()
    banker(player_one , displayed_diamond)
